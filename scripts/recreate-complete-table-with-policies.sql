-- ============================================
-- RECREAR TABLA COMPLETA CON TODAS LAS COLUMNAS Y POLÍTICAS
-- ============================================
...
-- 7. ELIMINAR POLÍTICAS EXISTENTES (si las hay)
DROP POLICY IF EXISTS "Enable read access for all users" ON consultas;
DROP POLICY IF EXISTS "Enable insert for authenticated users only" ON consultas;
DROP POLICY IF EXISTS "Enable update for users based on email" ON consultas;
DROP POLICY IF EXISTS "Enable delete for users based on user_id" ON consultas;
DROP POLICY IF EXISTS "Permitir lectura a usuarios autenticados" ON consultas;
DROP POLICY IF EXISTS "Permitir inserción a usuarios autenticados" ON consultas;
DROP POLICY IF EXISTS "Permitir actualización a usuarios autenticados" ON consultas;
DROP POLICY IF EXISTS "Permitir eliminación a usuarios autenticados" ON consultas;

-- 8. CREAR POLÍTICAS RLS COMPLETAS
-- Política para SELECT (lectura) - Permite leer todos los registros
CREATE POLICY "consultas_select_policy" ON consultas
FOR SELECT
USING (true);

-- Política para INSERT (crear) - Permite crear registros a cualquier usuario
CREATE POLICY "consultas_insert_policy" ON consultas
FOR INSERT
WITH CHECK (true);

-- Política para UPDATE (actualizar) - Permite actualizar cualquier registro
CREATE POLICY "consultas_update_policy" ON consultas
FOR UPDATE
USING (true)
WITH CHECK (true);

-- Política para DELETE (eliminar) - Permite eliminar cualquier registro
CREATE POLICY "consultas_delete_policy" ON consultas
FOR DELETE
USING (true);

-- 9. OTORGAR PERMISOS COMPLETOS A USUARIOS AUTENTICADOS Y ANÓNIMOS
GRANT ALL ON consultas TO authenticated;
GRANT ALL ON consultas TO anon;
GRANT ALL ON consultas TO service_role;

-- 10. OTORGAR PERMISOS EN EL ESQUEMA
...
-- 13. MOSTRAR TODAS LAS COLUMNAS CREADAS
SELECT 
    column_name, 
    data_type,
    is_nullable,
...
-- 17. PROBAR INSERCIÓN (OPCIONAL)
DO $$
BEGIN
    INSERT INTO consultas (id, usuario_creador, nombre_paciente) 
    VALUES ('TEST-001', 'test_user', 'Test Patient');
    
    RAISE NOTICE 'Inserción de prueba exitosa';
    
    DELETE FROM consultas WHERE id = 'TEST-001';
    RAISE NOTICE 'Eliminación de prueba exitosa';
EXCEPTION
    WHEN OTHERS THEN
...
-- ============================================
-- SCRIPT COMPLETADO CON POLÍTICAS RLS
-- ============================================

SELECT 'SCRIPT COMPLETADO - Tabla consultas recreada con todas las columnas y políticas RLS' as resultado;
