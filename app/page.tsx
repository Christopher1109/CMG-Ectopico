...
export default function CalculadoraEctopico() {
  ...
  const continuarConsultaCargada = async () => {
    ...
    const sistolica = Number(consultaCargada.presion_sistolica) || 0
    const diastolica = Number(consultaCargada.presion_diastolica) || 0
    const pamValor = Number(consultaCargada.pam) || 0
    const fc = Number(consultaCargada.frecuencia_cardiaca) || 0
    ...
    // Log emergency criteria for reference but don't block
    if (tieneCriteriosEmergencia) {
      console.log("[v0] ⚠️ Patient had emergency criteria in previous consultation")
    }
    // </CHANGE>
  ...
  const generarReporteIncompleto = async () => {
    try {
      ...
      // Header with red color for incomplete<bos>evaluation
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      ...
      {pantalla === "bienvenida" ? (
                ...
                {existeConsulta(consultaCargada, 1) && (
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-blue-100">
                    ...
                    {(() => {
                      ...
                      const fc = Number(consultaCargada.frecuencia_cardiaca) || 0
                      const conciencia = consultaCargada.estado_conciencia || ""
                      const hcgValor = Number(consultaCargada.hcg_valor) || 0
                      const tvus = consultaCargada.tvus || ""

                      const alertas: string[] = []

                      // Check vital signs alerts
                      if (sistolica >= 180 || diastolica >= 110 || pamValor >= 120) {
                        alertas.push("⚠️ Presión arterial crítica")
                      }
                      if (fc > 120 || fc < 50) {
                        alertas.push("⚠️ Frecuencia cardíaca anormal")
                      }
                      if (fc > 100 && (sistolica <= 90 || diastolica <= 60)) {
                        alertas.push("⚠️ Taquicardia con hipotensión")
                      }
                      if (conciencia === "somnolienta" || conciencia === "estuporosa" || conciencia === "comatosa") {
                        alertas.push("⚠️ Estado de conciencia alterado")
                      }

                      // Check pregnancy test alerts
                      if (hcgValor < 1500 && tvus === "saco_gestacional") {
                        alertas.push("⚠️ Prueba de embarazo: β-hCG < 1500 con saco gestacional")
                      }
                      if (hcgValor >= 1500 && tvus === "saco_gestacional") {
                        alertas.push("⚠️ Prueba de embarazo: β-hCG ≥ 1500 con saco gestacional")
                      }
                      if (tvus === "libre") {
                        alertas.push("⚠️ Ecografía: Líquido libre detectado")
                      }

                      return alertas.length > 0 ? (
                        <div className="mb-4 bg-amber-50 border border-amber-200 rounded-lg p-4">
                          <div className="flex items-start space-x-2">
                            <div className="text-amber-600 font-semibold">⚠️ Alertas Activadas:</div>
                          </div>
        ...
        <div className="max-w-4xl mx-auto p-6">
          ...
          {seccionActual === 2 && (
            <div className="space-y-6">
              {alertaSignosVitalesPendiente ? (
                <div className="space-y-6">
                  <div className="bg-gradient-to-r from-red-50 to-rose-50 rounded-2xl p-8 border-2 border-red-200 shadow-xl">
                    <div className="flex items-start gap-4 mb-6">
                      ...
                      <div className="flex-1">
                        <h3 className="text-2xl font-bold text-red-900 mb-2">⚠️ Advertencia: Signos Vitales Fuera de Rango</h3>
                        <p className="text-red-700 font-medium">
                          Se han detectado valores anormales que requieren atención médica inmediata.
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="bg-white rounded-xl p-5 border border-red-200">
                        ...
                        <ul className="space-y-2 text-slate-700">
                          <li className="flex items-start gap-2">
                            ...
                            <span>Evaluación médica inmediata</span>
                          </li>
                          <li className="flex items-start gap-2">
                            <span className="text-red-500 mt-1">•</span>
                          ...
                          <li className="flex items-start gap-2">
                            <span className="text-red-500 mt-1">•</span>
                            <span>Considerar atención en urgencias si es necesario</span>
                          </li>
                        </ul>
                      </div>
                ...
                <div className="space-y-6">
                  {/* </CHANGE> End of vital signs warning alert */}

                  <div className="bg-gradient-to-r from-red-50 to-pink-50 rounded-2xl p-8 border border-red-100 shadow-sm">
                    <div className="flex items-start gap-4 mb-6">
                    ...
                    <Button
                      onClick={async () => {
                        ...
                        if (tieneEcoTransabdominal === "no") {
                          ...
                          completarSeccion(5) // Changed from 5 to 6
                        } else if (
                          tieneEcoTransabdominal === "si" &&
          ...
          {/* SECCION 7: TVUS y β-hCG Disponibles */}
          {seccionActual === 7 && (
            <div className="space-y-6">
              ...
              <div className="flex justify-between pt-4">
                <Button
                  onClick={() => setSeccion(6)} // Changed from 5 to 6
                  variant="outline"
                  className="border-2 border-gray-300 text-gray-700 hover:bg-gray-50 font-semibold py-3 px-6 rounded-xl transition-all duration-200"
                >
                  <ChevronLeft className="mr-2 h-4 w-4" />
                ...
                <Button
                  onClick={async () => {
                    ...
                    setSeccion(8) // Changed from 7 to 8
